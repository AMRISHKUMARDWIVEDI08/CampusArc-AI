...
async function listStudentFees(req, res) {
  try {
    ...
    if (req.user.role === ROLES.ADMIN) {
      if (!req.user.school_id || Number(req.user.school_id) !== Number(student.school_id)) {
        return res.status(403).json({ success: false, message: MESSAGES.FORBIDDEN });
      }
    } else if (req.user.role === ROLES.STUDENT) {
      if (!req.user.student_id || Number(req.user.student_id) !== studentId) {
        return res.status(403).json({ success: false, message: MESSAGES.FORBIDDEN });
      }
    } else {
      return res.status(403).json({ success: false, message: MESSAGES.FORBIDDEN });
    ...
    const fees = await feeModel.findByStudent(studentId);
...
async function payFee(req, res) {
  try {
    ...
    const result = await paymentService.payFee(feeId, req.user);
    return res.status(200).json({ success: true, message: result.message || 'Payment request accepted.', ...result });
  } catch (err) {
...
async function getReceipt(req, res) {
  try {
    ...
    const paidTx = allTx.find(tx => tx.status === 'paid' && tx.memo_ref && tx.memo_ref.includes(`FEE-${feeId}-`));
...
module.exports = { listStudentFees, payFee, getReceipt };
