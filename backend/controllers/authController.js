...
async function register(req, res, next) {
  try {
    ...
    return res.status(201).json({ token, user: { id: user.id, email: user.email, name: user.name, role: user.role, school_id: user.school_id } });
  } catch (err) {
    return next(err);
  }
}
...
async function login(req, res, next) {
  try {
    ...
    const token = signToken(user);
    return res.json({ token, user: { id: user.id, email: user.email, name: user.name, role: user.role, school_id: user.school_id } });
  } catch (err) {
    return next(err);
  }
}
